...
from .._utils import color_background, add_suffix, rem_suffix, is_cross, cross_name, cross_split, make_header
import pandas as pd
import numpy as np
...
class WOE:
    ...
    def __init__(self, ds=None, features=None, scorecard=None, round_digits=3, rounding_migration_coef=0.001,
        ...
        self.feature_woes = {rem_suffix(f): self.create_feature_woe(ds, rem_suffix(f)) for f in features if not is_cross(f)} if features else {}
    ...
    @staticmethod
    def auto_fit_feature(feature_obj, auto_fit_parameters):
        """
        ...
        """
        return feature_obj.auto_fit(**auto_fit_parameters)
    ...
    def auto_fit(self, features=None, autofit_folder='auto_fit', plot_flag=-1, verbose=False,
        ...
        # feature_log, feature_gini, feature_bl, feature_woe, woe_out
        def save_check_dfs(dfs, woe_best_samples, check_file):
            ...
            def save_res_file(df, i, writer):
                ...
                if i == 0:
                    ...
                    df.to_excel(writer, sheet_name='Gini', index=False, float_format=f'%0.{self.round_woe}f')
                    worksheet = writer.sheets['Gini']
                    worksheet.column_dimensions['A'].width = 40
                    worksheet.column_dimensions['B'].width = 12
                    ...
                    df.to_excel(writer, sheet_name='WOE', index=False, float_format=f'%0.{self.round_woe}f')
            ...
            if dfs:
                with pd.ExcelWriter(check_file, engine="openpyxl") as writer, \
                    ...
                    save_res_file(df_params, 5, writer)
                    save_res_file(df_params, 5, writer_all)
        ...
        auto_fit_parameters = {}
        for f in ['method', 'max_n_bins', 'min_bin_size', 'criterion', 'scoring', 'max_depth', 'solver', 'divergence', 'pvalue',
                  ...
                  'BL_on', 'BL_allow_Vlogic_to_increase_gini', 'WOEO_on', 'WOEO_analytic', 'WOEO_all_samples']:
            auto_fit_parameters[f] = eval(f)
        auto_fit_parameters['verbose'] = verbose if self.ds_aux.n_jobs == 1 else False
        ...
        if features:
            ...
            for num_p, param in enumerate(params_list, start=1):
                ...
                if self.ds_aux.n_jobs > 1 and len(features) > self.ds_aux.n_jobs:
                    with futures.ProcessPoolExecutor(max_workers=self.ds_aux.n_jobs) as pool:
                        pool_map = pool.map(partial(self.auto_fit_feature, auto_fit_parameters=auto_fit_parameters),
                    ...
                    for feature in tqdm(features) if self.ds_aux.logger.level <= 20 else features:
                        self.feature_woes[feature].is_active, check_dfs[num_p][feature] = \
                            self.feature_woes[feature].auto_fit(**auto_fit_parameters)
                ...
                woe_dfs = [check_dfs[num_p][f][4] for f in check_dfs[num_p] if self.feature_woes[f].is_active]
                if woe_dfs:
            ...
            if excluded:
                self.ds_aux.logger.info(f'Excluded features {excluded} because no suitable binning was found for them')
        ...
        if cross_features or cross_num_second_level != 0:
            ...
            if cross_features:
                ...
                if cross_num_second_level == -1 or cross_num_second_level >= len(features_active) - 1:
                    ...
                    for f1 in tqdm(cross_features_first_level) if self.ds_aux.logger.level <= 20 else cross_features_first_level:
            ...
            for num_p, param in enumerate(params_list, start=1):
                ...
                if self.ds_aux.n_jobs > 1 and len(features) > self.ds_aux.n_jobs:
                    ...
                    for f1 in tqdm(crosses) if self.ds_aux.logger.level <= 20 else crosses:
                        checks_result[num_p][f1], check_dfs[num_p][f1] = self.feature_crosses[f1].auto_fit(**auto_fit_parameters)
    ...
    def plot_bins_feature(feature_woe, ds_aux, folder=None, plot_flag=True, show_groups=False, all_samples=False,
        ...
        :param pvalue: уровень значимости для отрисовки доверительных интервалов
        ...
        if ds_aux.time_column is not None:
            ...
            if 'Bootstrap std' in gini_in_time.columns:
                ax_3.fill_between(gini_in_time.index,
                                  gini_in_time['Bootstrap mean'] - z * gini_in_time['Bootstrap std'],
                                  gini_in_time['Bootstrap mean'] + z * gini_in_time['Bootstrap std'],
                                  alpha=0.1, color='blue', label=f'{(1 - pvalue) * 100}% conf interval')
        ...
        woe_df = feature_woe.calc_woe_confint(pvalue=pvalue, analytic=analytic, all_samples=all_samples).rename({'n': 'Observations_0', 'n1': 'target_0'}, axis=1)
        if ds_aux.id_column is not None:
            ...
            woe_df['Unique ids'] = woe_df['Observations_0']
        ...
        if all_samples:
            ...
            for i, sample in enumerate(samples_to_show[1:], start=1):
                ...
                woe_df = woe_df.merge(tmp.groupby('group').agg({ds.target: ['sum', 'size']}).set_axis([f'target_{i}', f'Observations_{i}'], axis=1).reset_index(), on='group', how='outer')
        woe_df[f'woe_{ds.train_name}'] = woe_df['woe']
        woe_df['down_trend'] = ((woe_df['woe'] < woe_df['woe'].shift(1)) & (woe_df['group'].shift(1) >= 0)) if not feature_woe.categorical_type else False
        woe_df['label'] = woe_df['group'].map(feature_woe.groups).astype('str').apply(lambda x: '\n'.join(wrap(str(x), 25)))
        woe_df['Target rate'] = (woe_df['target_0'] * 100 / woe_df['Observations_0']).round(2).astype('str') + '%'
        ...
        if feature_woe.missing_group != -1 and not feature_woe.categorical_type:
            woe_df.loc[woe_df.group == feature_woe.missing_group, 'label'] += '\nwith missings'
        if not feature_woe.categorical_type:
        ...
        for i, group in enumerate(feature_woe.special_bins.keys()):
            woe_df.loc[woe_df['group'] == -2 - i, 'label'] = group
        ...
        for i, sample in enumerate(samples_to_show):
            ...
            amt = woe_df[f'Observations_{i}'].sum()
            ax_1.bar(woe_df.index + shift, (woe_df[f'Observations_{i}'] - woe_df[f'target_{i}']) / amt, width=w, zorder=0, alpha=1 - i * 0.25, color='forestgreen', label=f'Class 0{" (" + sample + ")" if len(samples_to_show) > 1 else ""}')
            ax_1.bar(woe_df.index + shift, woe_df[f'target_{i}'] / amt, bottom=(woe_df[f'Observations_{i}'] - woe_df[f'target_{i}']) / amt, width=w, zorder=0, alpha=1 - i * 0.25, color='indianred', label=f'Class 1{" (" + sample + ")" if len(samples_to_show) > 1 else ""}')
        ...
        ax2.errorbar(woe_df.index + shift, (woe_df['woe_lower'] + woe_df['woe_upper']) / 2, xerr=0, yerr=(woe_df['woe_upper'] - woe_df['woe_lower']) / 2, color='b', capsize=5, fmt='none', alpha=0.4)
    ...
    @staticmethod
    def plot_bins_feature_cross(params, ds_aux, folder=None, plot_flag=True, show_groups=False, all_samples=False):
        """
        Отрисовывает биннингодной переменной
        :param feature_woe: объект класса FeatureWOE
        :param ds_aux: вспомогательный ДатаСэмпл с полем среза
        ...
        if ds_aux.id_column is not None:
            ...
            woe_df = to_calc.groupby('group').agg({ds.target: ['sum', 'size']}).set_axis(['target_0', 'Observations_0'], axis=1)
            woe_df['Unique ids'] = woe_df['Observations_0']
            woe_df['Class 1 unique ids'] = woe_df['target_0']
        woe_df = woe_df.reset_index()
        samples_to_show = [ds.train_name]
        ...
        woe_df['Target rate'] = (woe_df['target_0'] * 100 / woe_df['Observations_0']).round(1).astype('str') + '%'
        ...
        woe_df['down_trend'] = ((woe_df['woe_0'] < woe_df['woe_0'].shift(1)) & (
                woe_df['group1'] == woe_df['group1'].shift(1)) & (woe_df['group2'].shift(1) >= 0)) if not feature_woe2.categorical_type else False
        woe_df = woe_df.fillna(0)
        for g in feature_woe.groups:
            if feature_cross.cross[f2][g].missing_group != -1 and not feature_cross.cross[f2][g].categorical_type:
                woe_df.loc[(woe_df['group1'] == g) & (
                        woe_df['group2'] == feature_cross.cross[f2][g].missing_group), 'label'] += '\nwith missings'
        ...
        ax_1.set_xticklabels(woe_df['label'], rotation=30, ha='right', fontsize=size - 1 if show_groups else size)
        ...
        for i, sample in enumerate(samples_to_show):
            ...
            shift = (i - (len(samples_to_show) - 1) / 2) * (w + 0.03)
            amt = woe_df[f'Observations_{i}'].sum()
            ax_1.bar(woe_df['coord'] + shift, (woe_df[f'Observations_{i}'] - woe_df[f'target_{i}']) / amt, width=w,
            ...
            ax_1.bar(woe_df['coord'] + shift, woe_df[f'target_{i}'] / amt,
                     bottom=(woe_df[f'Observations_{i}'] - woe_df[f'target_{i}']) / amt, width=w, zorder=0,
            ...
            if i == 0 and feature_woe.ds.bootstrap_base is not None:
                ...
                bootstrap_woes = {g: np.array([w[g] for w in bootstrap_woes_l if g in w]) for g in
                                  tmp['group'].unique()}
                woe_df['woe_mean'] = woe_df['group'].map({g: np.mean(v) for g, v in bootstrap_woes.items()})
                woe_df['woe_std'] = woe_df['group'].map({g: np.std(v) for g, v in bootstrap_woes.items()})
                ax2.errorbar(woe_df['coord'] + shift, woe_df['woe_mean'], xerr=0, yerr=woe_df['woe_std'] * 1.96, color='b',
        ...
        if self.ds_aux.n_jobs > 1 and len(features) > self.ds_aux.n_jobs:
            with futures.ProcessPoolExecutor(max_workers=self.ds_aux.n_jobs) as pool:
                pool_iter = pool.map(partial(self.plot_bins_feature, ds_aux=self.ds_aux, folder=folder, plot_flag=plot_flag,
                                             show_groups=show_groups, all_samples=all_samples, pvalue=pvalue, analytic=analytic),
    ...
    def transform(self, ds, features=None, verbose=False):
        ...
        for f in features:
            ...
            if f in self.feature_woes and self.feature_woes[f].is_active:
                ...
                if verbose:
                    self.ds_aux.logger.warning(f"Can't transform feature {f}. Skipped.")
    ...
    def import_scorecard(self, scorecard, features=None, verbose=True, fit_flag=False):
        ...
        for feature in features:
            if not is_cross(feature):
                if feature in self.feature_woes:
                    if verbose:
                        self.ds_aux.logger.info(f'Replacing binning for {feature}..')
                    self.feature_woes[feature].import_scorecard(scorecard[scorecard['feature'] == feature], fit_flag=fit_flag)
                else:
                    self.ds_aux.logger.warning(f'Feature {feature} is missing in self.feature_woes! Skipped.')
        ...
        if not scorecard.empty:
            ...
            for f1, scorecard_f1 in scorecard.groupby('f1'):
                if verbose:
                    self.ds_aux.logger.info(f'Replacing binning for cross features {f1}..')
                if f1 in self.feature_woes:
                    fw = copy.deepcopy(self.feature_woes[f1])
                else:
                    fw = self.create_feature_woe(DataSamples(logger=None, verbose=False), f1)
                ...
                self.feature_crosses[f1].add_f2_features([self.feature_woes[f2] if f2 in self.feature_woes else self.create_feature_woe(DataSamples(logger=None, verbose=False), f2)
    ...
    def calc_woe_confint(self, features=None, pvalue=0.05, analytic=False, all_samples=False):
        """
        ...
        :param pvalue: уровень значимости
        :param analytic: всегда считать доверительные интервалы аналитически
        ...
        if features is None:
            features = list(self.feature_woes)
        return pd.concat([self.feature_woes[f].calc_woe_confint(pvalue=pvalue, analytic=analytic, all_samples=all_samples).assign(**{'feature': f}) for f in features if f in self.feature_woes])
...
class FeatureWOE:
    ...
    def __init__(self, ds, feature, round_digits=3, round_woe=3, rounding_migration_coef=0.001,
        ...
        self.ds = DataSamples(samples=samples, target=ds.target, features=[feature], cat_columns=[feature] if feature in ds.cat_columns else [],
                              n_jobs=1, random_state=ds.random_state, logger=ds.logger, verbose=False)
        if ds.bootstrap_base is not None:
    ...
    def BusinessLogicChecker(self, BL_allow_Vlogic_to_increase_gini=100):
        '''
            ...
            if groups_info.shape[0] == 1:
                ...
                if re.fullmatch('-*|\+*', trend):
                    ...
                    if gini < BL_allow_Vlogic_to_increase_gini:
                        trend_type = 'no trend'
                        self.ds.logger.info(f'WOE trend is V-shaped, but gini is too low ({gini} < {BL_allow_Vlogic_to_increase_gini})')
                    else:
                        trend_type = 'V-shape'
    ...
    def GiniChecker(self, gini_threshold=5, gini_decrease_threshold=0.2, gini_increase_restrict=True, with_test=False,
                    pvalue=0.05):
        '''
        ...
        for name, gini in gini_values.items():
            ...
            if name != self.ds.train_name and with_test:
                ...
                if decrease > gini_decrease_threshold or (gini_increase_restrict and -decrease > gini_decrease_threshold):
                    gini_correct = False
                    self.ds.logger.info(f'Gini change from {self.ds.train_name} to {name} is greater then threshold: {round(decrease, self.round_woe)} > {gini_decrease_threshold}')
        if self.ds.bootstrap_base is not None and gini_correct:
            ...
            gini_5 = gini_values['Bootstrap mean'] - z * gini_values['Bootstrap std']
            if gini_5 < gini_threshold:
                gini_correct = False
                self.ds.logger.info(f'Less then 95% of gini distribution is greater then threshold: (mean-{z}*std) {round(gini_5, self.round_woe)} < {gini_threshold}')
            if gini_decrease_threshold < 1:
                decrease = z * gini_values['Bootstrap std']/gini_values['Bootstrap mean']
            else:
                decrease = z * gini_values['Bootstrap std']
            if decrease > gini_decrease_threshold:
                gini_correct = False
                if gini_decrease_threshold < 1:
                    self.ds.logger.info(f'Gini deviation from mean for 95% of distribution is greater then threshold: ({z}*std/mean) {round(decrease,self.round_woe)} > {gini_decrease_threshold}')
                else:
                    self.ds.logger.info(f'Gini deviation for 95% of distribution is greater then threshold: ({z}*std) {round(decrease, self.round_woe)} > {gini_decrease_threshold}')
        self.ds.logger.info('\n' + pd.DataFrame(gini_values, index=['Gini']).round(self.round_woe).to_string())
        self.ds.logger.info('...Passed!' if gini_correct else '...Failed!')
        return gini_correct, gini_values

    def calc_woe_confint(self, pvalue=0.05, analytic=False, all_samples=False):
        """
        Вычисляет доверительные интервалы WOE.
        Если заданы бутстреп сэмплы, то доверительные интервалы вычисляются бутсрепом, иначе  - аналитически
        :param pvalue: уровень значимости
        ...
        if analytic or self.ds.bootstrap_base is None:
            pu, pl = zip(*df.apply(lambda row: proportion_confint(row['n1'], row['n'], alpha=pvalue, method="wilson"), axis=1))
            alpha = self.alpha if self.alpha is not None else 0
            df['woe_lower'] = (np.log(df['n1'].sum() / df['n0'].sum() * (alpha + df['n']) /
                                      (df['n'] * (pl * df['n'] + self.woe_adjust) / (
                                              (1 - np.array(pl)) * df['n'] + self.woe_adjust) + alpha))).round(self.round_woe)
            df['woe_upper'] = (np.log(df['n1'].sum() / df['n0'].sum() * (alpha + df['n']) /
                                      (df['n'] * (pu * df['n'] + self.woe_adjust) / (
                                              (1 - np.array(pu)) * df['n'] + self.woe_adjust) + alpha))).round(self.round_woe)
        else:
            ...
            bootstrap_woes = {g: np.array([w[g] for w in bootstrap_woes_l if g in w]) for g in self.groups}
            df['woe_bootstrap_mean'] = df.index.map({g: np.mean(v) for g, v in bootstrap_woes.items()})
            woe_std = df.index.map({g: np.std(v) for g, v in bootstrap_woes.items()})
            df['woe_lower'] = df['woe_bootstrap_mean'] - stats.norm.ppf(1 - pvalue / 2) * woe_std
            df['woe_upper'] = df['woe_bootstrap_mean'] + stats.norm.ppf(1 - pvalue / 2) * woe_std
    ...
    def WOEOrderChecker(self, analytic=False, all_samples=False, pvalue=0.05):
        """
        ...
        woe_df = self.calc_woe_confint(pvalue=pvalue, analytic=analytic, all_samples=all_samples)
        self.ds.logger.info('\n' + woe_df.to_string())
        result = True
        if all_samples:
    ...
    def auto_fit(self, verbose=True, method='tree', max_n_bins=10, min_bin_size=0.05,
                 ...
                 WOEO_on=True, WOEO_analytic=True, WOEO_all_samples=False, pvalue=0.05):
        '''
        ...
        WOEO_analytic: всегда считать доверительные интервалы аналитически
        ...
        for n_bins in range(max_n_bins, 1, -1):
            ...
            try:
                self.fit(new_groups=True, to_history=False, method=method, max_n_bins=n_bins, min_bin_size=min_bin_size,
                         monotonic=BL_on and (BL_allow_Vlogic_to_increase_gini == 100) or (gini_V > 0),
            ...
            checks[4].append(self.export_scorecard(iteration=iteration))
            ...
            if BL_on:
                try:
                    BL_trend_type, BL_check = self.BusinessLogicChecker(BL_allow_Vlogic_to_increase_gini=BL_allow_Vlogic_to_increase_gini if gini_V == 0 else 100)
                except Exception as e:
            ...
            if G_on:
                try:
                    correct, gini_values = self.GiniChecker(gini_threshold=G_gini_threshold,
                                                            ...
                                                            gini_increase_restrict=G_gini_increase_restrict, 
            ...
            if WOEO_on:
                try:
                    correct, woe_check = self.WOEOrderChecker(analytic=WOEO_analytic, 
                                                              all_samples=WOEO_all_samples, 
                                                              pvalue=pvalue)
                except Exception as e:
            ...
            if gini_V > 0:
                ...
                if gini_delta > BL_allow_Vlogic_to_increase_gini:
                    ...
                    self.ds.logger.info(f'Finally V-shaped binning with {fact_number_groups_V} groups is chosen, because it gives an increase gini by {round(gini_delta, self.round_woe)}')
                ...
                for i in range(5):
                    if len(check_dfs_V[i]) > 0:
                        if i in [0, 1]:
                            ...
                            check_dfs_V[i]['iteration'] = iteration
                        checks[i].append(check_dfs_V[i])
                self.ds.logger.info(f'Finally V-shaped binning with {fact_number_groups_V} groups is chosen, because other binnings do not pass checks')
                self.ds.logger.setLevel(self.ds.logger_level)
                return True, checks_to_dfs(checks)
            else:
                if verbose:
    ...
    def woe_fit(self, missings_to_process=True):
        '''
    ...
    def fit(self, new_groups=True, to_history=True, method='tree', max_n_bins=10, min_bin_size=0.05, criterion='entropy',
            scoring='neg_log_loss', max_depth=None, monotonic=False, solver='cp', divergence='iv', verbose=False):
        '''
        ...
        self.set_groups(inplace=True)
        self.woe_fit(missings_to_process=missings_to_process)
        ...
        if len(self.groups) != len(self.woes):
            if (-1 in self.woes and -1 not in self.groups) and pd.isnull(self.woes[-1]):
                ...
                del self.groups[-1]
            self.ds.logger.warning(f'Number of groups is not certain! We have {len(self.groups)} groups and {len(self.woes)} woes!')
        if verbose:
            self.print_woe()
...
class FeatureCross:
        ...
        for fw in feature_woes:
            ...
            for group in self.feature_woe.groups:
                if fw.ds.samples is not None:
                    ...
                    samples = None
                fw2 = FeatureWOE(DataSamples(samples=samples, target=fw.ds.target, features=fw.ds.features, cat_columns=fw.ds.cat_columns,
                                             n_jobs=1, random_state=fw.ds.random_state, logger=fw.ds.logger, verbose=False),
    ...
    def auto_fit(self, cross_features=None, verbose=True, method='tree', max_n_bins=10, min_bin_size=0.05,
                 criterion='entropy', scoring='neg_log_loss', max_depth=None, solver='cp', divergence='iv', pvalue=0.05,
                 WOEM_on=True, WOEM_woe_threshold=0.05, WOEM_with_missing=False,
                 SM_on=True, SM_target_threshold=5, SM_size_threshold=100,
                 BL_on=True, BL_allow_Vlogic_to_increase_gini=100,
                 G_on=True, G_gini_threshold=5, G_with_test=False, G_gini_decrease_threshold=0.2, G_gini_increase_restrict=True,
                 WOEO_on=False, WOEO_analytic=True, WOEO_all_samples=False
                 ):
        auto_fit_parameters = {}
        for f in ['method', 'max_n_bins', 'min_bin_size', 'criterion', 'scoring', 'max_depth', 'solver', 'divergence', 'pvalue',
                  'WOEM_on', 'WOEM_woe_threshold', 'WOEM_with_missing', 'SM_on', 'SM_target_threshold', 'SM_size_threshold',
        ...
        for f2 in cross_features:
            res_group = {}
            for group in self.cross[f2]:
                res_group[group], check_dfs[f2][group] = self.cross[f2][group].auto_fit(**auto_fit_parameters)
                for i in range(5):
    ...
    def calc_simple_woes(self, f2):
        to_calc = self.feature_woe.ds.samples[self.feature_woe.ds.train_name]\
            .merge(pd.concat([fw.ds.samples[fw.ds.train_name]['group'] for g, fw in self.cross[f2].items()]),
                   left_index=True, right_index=True)
            ...
            for group, scorecard_f2_group in scorecard_f2.groupby('group1'):
                if group != 'others':
                    self.cross[f2][group].import_scorecard(scorecard=scorecard_f2_group[['f2', 'categorical_type2', 'group2', 'values2', 'woe', 'missing2']]\
                                            .set_axis(['feature', 'categorical_type', 'group', 'values', 'woe', 'missing'], axis=1), fit_flag=False)
