...
from .._utils import fig_to_excel, adjust_cell_width, add_suffix, rem_suffix, is_cross, cross_name, cross_split, add_ds_folder, make_header
import pandas as pd
import numpy as np
...
class LogisticRegressionModel:
    ...
    def __init__(self, clf=None, ds=None, transformer=None, round_digits=3, name=None):
        ...
        self.scale = None
        self.clf = clf if clf is not None else SGDClassifier(loss='log', penalty='l2', max_iter=1000, alpha=0.001,
    ...
    def auto_logreg(self, data=None, target=None, time_column=None, id_column=None, feature_descriptions=None, n_jobs=None,
        ...
        if data is not None:
            if isinstance(data, pd.DataFrame):
                self.ds = DataSamples(samples={'Train': data}, target=target, time_column=time_column, id_column=id_column,
                                      feature_descriptions=feature_descriptions, result_folder=result_folder,
                                      n_jobs=n_jobs, samples_split={})
            else:
        ...
        if not self.coefs:
            ...
            self.mfa(gini_threshold=10, result_file=out)
        else:
    ...
    def mfa(self, ds=None, features=None, hold=None, features_ini=None, limit_to_add=100, gini_threshold=5, psi_threshold=0.25,
        ...
        dropped_df, drop_features_corr, selection_fig, regularized_fig = \
            self.mfa_steps(features=features, hold=hold, gini_threshold=gini_threshold, psi_threshold=psi_threshold, corr_method=corr_method,
                           corr_threshold=corr_threshold, drop_with_most_correlations=drop_with_most_correlations,
                           drop_iteratively=drop_corr_iteratively, selection_type=selection_type, pvalue_threshold=pvalue_threshold,
                           pvalue_priority=pvalue_priority, scoring=scoring, score_delta=score_delta, n_stops=n_stops, cv=cv,
                           drop_positive_coefs=drop_positive_coefs, features_ini=features_ini, limit_to_add=limit_to_add,
                           verbose=verbose, gini_df=gini_df)
        if not crosses_simple and crosses_max_num > 0 and cross_features:
            if verbose:
                self.ds.logger.info(make_header('MFA for cross features', 125))
            cross_dropped_df, cross_drop_features_corr, cross_selection_fig, cross_regularized_fig = \
                self.mfa_steps(features=cross_features, hold=hold, gini_threshold=gini_threshold, psi_threshold=psi_threshold, corr_method=corr_method,
                               corr_threshold=corr_threshold, drop_with_most_correlations=drop_with_most_correlations,
                               drop_iteratively=drop_corr_iteratively, selection_type='stepwise', pvalue_threshold=pvalue_threshold,
                               pvalue_priority=pvalue_priority, scoring=scoring, score_delta=score_delta, n_stops=n_stops, cv=cv,
                               drop_positive_coefs=drop_positive_coefs, features_ini=self.features,
                               limit_to_add=crosses_max_num, verbose=verbose, gini_df=gini_df)
        else:
            cross_dropped_df, cross_drop_features_corr, cross_selection_fig, cross_regularized_fig = [], {}, None, None
        if result_file is not None and isinstance(result_file, (str, pd.ExcelWriter)):
            ...
            self.ds.corr_mat(sample_name=self.ds.train_name, features=features, description_df=gini_df, styler=True).to_excel(writer, sheet_name='Correlation analysis')
            adjust_cell_width(writer.sheets['Correlation analysis'], gini_df)
            add_figs = [selection_fig, regularized_fig]
            if not crosses_simple and crosses_max_num > 0 and cross_features:
                cross_dropped_df.to_excel(writer, sheet_name='Cross dropped features')
                adjust_cell_width(writer.sheets['Cross dropped features'], gini_df)
                gini_df['Drop reason'] = gini_df.index.map(cross_drop_features_corr)
                self.ds.corr_mat(sample_name=self.ds.train_name, features=cross_features, description_df=gini_df, styler=True).to_excel(writer, sheet_name='Cross Correlation analysis')
            ...
            self.report(out=writer, sheet_name=selection_type, pvalue_threshold=pvalue_threshold, verbose=verbose,
                        add_figs=add_figs, gini_df=gini_df.drop(['Drop reason'], axis=1),
                        metrics=metrics, metrics_cv=metrics_cv, cv=cv)
            if isinstance(result_file, str):
                writer.close()

    def mfa_steps(self, features=None, hold=None, gini_threshold=5, psi_threshold=0.25, verbose=False,
                  corr_method='pearson', corr_threshold=0.75, drop_with_most_correlations=False, drop_iteratively=False,
                  selection_type='stepwise', pvalue_threshold=0.05, pvalue_priority=False,
                  scoring='gini', score_delta=0.1, n_stops=1, cv=None, drop_positive_coefs=True,
                  features_ini=None, limit_to_add=100, gini_df=None):
        """
        Технический метод для выполнения всех шагов отбора в методе .mfa(), параметры полностью аналогичны
        """
        if features is None:
            features = self.ds.features.copy()
        ...
        if psi_threshold < 1:
            psi_df = self.ds.psi(features=features)[0]
            drop_features_psi = psi_df[psi_df.max(axis=1) > psi_threshold].index.to_list()
        ...
        if not drop_iteratively:
            ...
            drop_features_corr = {}
            corr_df = self.ds.samples[self.ds.train_name][list(set(features) | features_ini)].corr(method=corr_method).abs() > corr_threshold
        if verbose:
        ...
        return dropped_df, drop_features_corr, selection_fig, regularized_fig
    ...
    def report(self, ds=None, out='report.xlsx', sheet_name=None, pvalue_threshold=0.05, verbose=False, add_figs=None,
        ...
        if add_figs:
            ...
            for fig in add_figs:
                if fig:
                    ...
                    row = +16
        if verbose:
            ds.logger.info('\n' + model_gini.rename(index={score_field: 'Gini'}).to_string())
        ...
        if 'psi' in metrics:
            result, figs = ds.psi(features=self.features, scorecard=scorecard, styler=True)
            result.to_excel(writer, sheet_name='PSI_samples')
            ws = writer.sheets['PSI_samples']
            ws.set_column(0, 0, 40)
            ws.set_column(1, len(result.columns) + 1, 12)
            for i, fig in enumerate(figs):
                fig_to_excel(fig, ws, row=i * (21 + 1) + len(self.features) + 3, col=0, scale=0.9)
            if ds.time_column:
                result, figs = ds.psi(time_column=ds.time_column, features=self.features, scorecard=scorecard, styler=True)
    ...
    def regularized_selection(self, ds=None, features=None, hold=None, scoring='gini', cv=None, pvalue_threshold=0.05, verbose=False):
        ...
        while to_refit:
            ...
            if positive_to_exclude:
                ...
                if verbose:
                    self.ds.logger.info(f'To drop: {to_exclude}, gini: {ginis[to_exclude]}, coef: {positive_to_exclude[to_exclude]}')
                ...
                if feature_to_exclude_array:
                    to_refit = True
                    if verbose:
                        self.ds.logger.info(f'To drop: {feature_to_exclude_array[0]}, gini: {ginis[feature_to_exclude_array[0]]}, p-value: {wald[wald.index == feature_to_exclude_array[0]]["p-value"].values[0]}')
    ...
    def add_feature_stat(df, clf, scoring, cv):
        ...
        try:
            ...
            return 1, 0
        return score, pvalue[1]

    def add_feature(self, features=None, candidates=None, scoring='gini', cv=None, corr_df=None):
        """
        ...
        if self.ds.n_jobs > 1:
                ...
                while iterations:
                    for feature in candidates_iter:
                        jobs[pool.submit(self.add_feature_stat, df=self.ds.samples[self.ds.train_name][[self.ds.target, feature] + list(features)],
                                         clf=self.clf, scoring=scoring, cv=cv)] = feature
            ...
            for feature in candidates:
                ...
                self.clf.fit(self.ds.samples[self.ds.train_name][tmp_features],
                             self.ds.samples[self.ds.train_name][self.ds.target])
                score = self.get_score(scoring=scoring, cv=cv, features=tmp_features, fit=False)
                try:
                    wald = self.wald_test(features=tmp_features, clf=self.clf, fit=False)
                    pvalue = wald[wald.index == feature]['p-value'].values[0]
                except:
                    pvalue = 1
                result_list.append([feature, score, pvalue])
        return pd.DataFrame(result_list, columns=['feature', 'score', 'p-value']).sort_values(by=['score'], ascending=False)

    def drop_feature(self, features=None, scoring='gini', cv=None):
        """
        ...
        if cv is None:
            cv = StratifiedKFold(5, shuffle=True, random_state=self.ds.random_state)
        return pd.DataFrame([[f, self.get_score(scoring=scoring, cv=cv, features=list(features - {f}), fit=True)]
                             for f in features], columns=['feature', 'score']).sort_values(by=['score'], ascending=False)\
              .merge(self.wald_test(clf=self.clf, features=list(features), fit=True).reset_index()[['feature', 'p-value']],
                     on=['feature'], how='left')
    ...
    def stepwise_selection(self, ds=None, verbose=False, selection_type='stepwise', features=None, hold=None,
        ...
        def get_add_feature(features, candidates, score, n_stops):
            ...
            if not df_scores.empty:
                if pvalue_priority:
                    result = tuple(df_scores[df_scores['p-value'] == df_scores['p-value'].min()].values[0])
                    if result[2] > pvalue_threshold:
                        n_stops -= 1
                else:
        ...
        def get_drop_feature(features, hold):
            wald = self.wald_test(clf=self.clf, features=list(features), fit=True)
            ...
            score = self.get_score(scoring=scoring, cv=cv, features=list(features - {feature}), fit=True)
        ...
        self.ds.logger.info(f'{selection_type.capitalize()} feature selection started...')

        # Forward selection
        if selection_type == 'forward':
            for i in range(len(candidates)):
                feature, score, pvalue, n_stops = get_add_feature(features, candidates, score, n_stops)
                if feature is None:
                    break
                self.ds.logger.info(f'To add: {feature}, {scoring}: {score}, p-value: {pvalue}')
        ...
        # Backward selection
            ...
            for i in range(len(candidates)):
                feature, score, pvalue = get_drop_feature(features, hold)
                if feature is None:
                    break
                self.ds.logger.info(f'To drop: {feature}, {scoring}: {score}, p-value: {pvalue}')
        ...
        # stepwise selection
            ...
            for i in range(len(candidates)):
                ...
                if features:
                    feature, drop_score, pvalue = get_drop_feature(features, hold)
                    if feature:
                        score = drop_score
                        self.ds.logger.info(f'To drop: {feature}, {scoring}: {score}, p-value: {pvalue}')
                        features.remove(feature)
    ...
    def scoring(self, data=None, score_field='score', pd_field='pd', scale_field=None):
        ...
        if not self.coefs or not self.intercept:
            self.print_log(f'No calculated intercept and coefs in self, use self.fit() before. Return None')
        if data is not None:
            if isinstance(data, pd.DataFrame):
