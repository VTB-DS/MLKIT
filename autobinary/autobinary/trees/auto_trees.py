from sklearn.metrics import roc_curve, roc_auc_score, accuracy_score, f1_score, recall_score, precision_score, classification_report
...
class AutoTrees():
    ...
    def _get_metric(self, y_true, y_pred, metric:str):
        ...
        if metric=='accuracy':
            ...
            result = roc_auc_score(y_true, y_pred, multi_class='ovr')
    ...
    def _calc_main_metric(self, X_train, X_test, y_train, y_test)-> dict:
        ...
        Returns:
            dict: [description]
        """
        if self.main_metric in ['accuracy', 'mae', 'mse', 'rmse', 'mape', 'f1_macro', 'f1_micro', 'f1_weighted', 'precision_macro', 'precision_micro', 'precision_weighted', 'recall_macro', 'recall_micro', 'recall_weighted']:
            ...
            elif self.model_type in ['catboost', 'catboost_gpu']:
        ...
        if self.main_metric in ['roc_auc_ovr', 'roc_auc_ovo']:
            if self.model_type == 'xgboost':
    ...
    def _extra_metrics_multiclassif(self, X_train, X_test, y_train, y_test)-> dict:
        """
        ...
        Returns:
            dict: [description]
        """
        if self.main_metric in ['f1_macro', 'f1_micro', 'f1_weighted', 'precision_macro', 'precision_micro', 'precision_weighted', 'recall_macro', 'recall_micro', 'recall_weighted']:
            ...
            f1macro_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='f1_macro')
            f1macro_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='f1_macro')

            f1micro_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='f1_micro')
            f1micro_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='f1_micro')

            f1w_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='f1_weighted')
            f1w_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='f1_weighted')
            
            prmacro_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='precision_macro')
            prmacro_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='precision_macro')

            prmicro_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='precision_micro')
            prmicro_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='precision_micro')

            prw_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='precision_weighted')
            prw_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='precision_weighted')
            
            recmacro_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='recall_macro')
            recmacro_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='recall_macro')

            recmicro_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='recall_micro')
            recmicro_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='recall_micro')

            recw_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='recall_weighted')
            recw_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='recall_weighted')
         
            dict_temp = {'f1_macro_train': f1macro_train, 'f1_micro_train': f1micro_train, 'f1_weighted_train': f1w_train, 'precision_macro_train': prmacro_train, 'precision_micro_train': prmicro_train, 'precision_weighted_train': prw_train, 'recall_macro_train': recmacro_train, 'recall_micro_train': recmicro_train, 'recall_weighted_train': recw_train, 
                         'f1_macro_valid': f1macro_valid, 'f1_micro_valid': f1micro_valid, 'f1_weighted_valid': f1w_valid, 'precision_macro_valid': prmacro_valid, 'precision_micro_valid': prmicro_valid, 'precision_weighted_valid': prw_valid, 'recall_macro_valid': recmacro_valid, 'recall_micro_valid': recmicro_valid, 'recall_weighted_valid': recw_valid}
            ...
            auc_ovr_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='roc_auc_ovr')
            auc_ovr_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='roc_auc_ovr')

            auc_ovo_train = self._get_metric(y_true=y_train, y_pred=y_pred_train,metric='roc_auc_ovo')
            auc_ovo_valid = self._get_metric(y_true=y_test, y_pred=y_pred_test,metric='roc_auc_ovo')

            dict_temp = {'auc_ovr_train': auc_ovr_train, 'auc_ovo_train': auc_ovo_train, 
                         'auc_ovr_valid': auc_ovr_valid, 'auc_ovo_valid': auc_ovo_valid}
    ...
    def _solo_preprocessing(self, X_tr, X_val, y_tr, y_val, trt_tr=None, trt_val=None):
        ...
        X_tr, X_val, y_tr, y_val = self._preprocessing(
                X_tr=X_tr, X_val=X_val, y_tr=y_tr, y_val=y_val, main='main')
    ...
    def _two_preprocessing(self, X_tr, X_val, y_tr, y_val, trt_tr=None, trt_val=None):
        ...
        if self.uplift == 'two_vanilla_ctrl':
            ...
            # ограничиваем conrol выборку
            X_ctrl_tr = X_ctrl_tr[self.extra_features]
            X_ctrl_val = X_ctrl_val[self.extra_features]
            
            # обрабатываем conrol выборку
            X_ctrl_tr, X_ctrl_val, y_ctrl_tr, y_ctrl_val = self._preprocessing(
                X_tr=X_ctrl_tr, X_val=X_ctrl_val, y_tr=y_ctrl_tr, y_val=y_ctrl_val, main='extra')
            ...
            X_trt_tr, X_trt_val, y_trt_tr, y_trt_val = self._preprocessing(
                X_tr=X_trt_tr, X_val=X_trt_val, y_tr=y_trt_tr, y_val=y_trt_val, main='main')
            ...
            ddr_control_tr = self.extra_estimator.predict_proba(X_trt_tr[self.extra_features])[:, 1]
            ddr_control_val = self.extra_estimator.predict_proba(X_trt_val[self.extra_features])[:, 1]
            ...
            # ограничиваем treatment выборку
            X_trt_tr = X_trt_tr[self.extra_features]
            X_trt_val = X_trt_val[self.extra_features]
            
            # обрабатываем treatment выборку
            X_trt_tr, X_trt_val, y_trt_tr, y_trt_val = self._preprocessing(
                X_tr=X_trt_tr, X_val=X_trt_val, y_tr=y_trt_tr, y_val=y_trt_val, main='extra')

            # выделяем control выборку
            X_ctrl_tr, y_ctrl_tr = X_tr[trt_tr == 0], y_tr[trt_tr == 0]
            X_ctrl_val, y_ctrl_val = X_val[trt_val == 0], y_val[trt_val == 0]

            # обрабатываем control выборку
            X_ctrl_tr, X_ctrl_val, y_ctrl_tr, y_ctrl_val = self._preprocessing(
                X_tr=X_ctrl_tr, X_val=X_ctrl_val, y_tr=y_ctrl_tr, y_val=y_ctrl_val, main='main')
            ...
            ddr_treatment_tr = self.extra_estimator.predict_proba(X_ctrl_tr[self.extra_features])[:, 1]
            ddr_treatment_val = self.extra_estimator.predict_proba(X_ctrl_val[self.extra_features])[:, 1]
    ...
    def _model_fit(self, X_train: pd.DataFrame, y_train: pd.DataFrame, X_val=None, y_val=None,
        ...
        if self.uplift is None:
            X_train_new, X_val_new, y_train, y_val = self._preprocessing(
                X_tr=X_train, X_val=X_val, y_tr=y_train, y_val=y_val, main='main')
        ...
        elif self.main_metric in ['f1_macro', 'f1_micro', 'f1_weighted', 'precision_macro', 'precision_micro', 'precision_weighted', 'recall_macro', 'recall_micro', 'recall_weighted', 'roc_auc_ovr', 'roc_auc_ovo']:
            metr = self._extra_metrics_multiclassif(X_train = X_train_new, X_test=X_val_new, y_train=y_train, y_test=y_val)
    ...
    def get_extra_scores(self)->pd.DataFrame:
        ...
        if self.main_metric in ['roc_auc', 'accuracy', 'gini', 'delta_gini']:
            ...
            if self.main_metric in ['f1_macro','precision_macro','recall_macro']:
                df['names'] = ['f1_macro_train', 'f1_macro_valid','precision_macro_train', 'precision_macro_valid','recall_macro_train', 'recall_macro_valid']

                for i in range(1, len(self._extra_scores)+1):
                    val_metrics = []
                    val_metrics.append(self._extra_scores[f'scores_{i}']['f1_macro_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['f1_macro_valid'])
                    
                    val_metrics.append(self._extra_scores[f'scores_{i}']['precision_macro_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['precision_macro_valid'])
                    
                    val_metrics.append(self._extra_scores[f'scores_{i}']['recall_macro_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['recall_macro_valid']) 
                    df[f'fold_{i}'] = val_metrics  
                    
            elif self.main_metric in ['f1_micro','precision_micro','recall_micro']:
                df['names'] = ['f1_micro_train', 'f1_micro_valid', 'precision_micro_train', 'precision_micro_valid', 'recall_micro_train', 'recall_micro_valid']

                for i in range(1, len(self._extra_scores)+1):
                    val_metrics = []
                    val_metrics.append(self._extra_scores[f'scores_{i}']['f1_micro_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['f1_micro_valid'])

                    val_metrics.append(self._extra_scores[f'scores_{i}']['precision_micro_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['precision_micro_valid'])

                    val_metrics.append(self._extra_scores[f'scores_{i}']['recall_micro_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['recall_micro_valid'])
                    df[f'fold_{i}'] = val_metrics  
                    
            elif self.main_metric in ['f1_weighted','precision_weighted','recall_weighted']:
                df['names'] = ['f1_weighted_train', 'f1_weighted_valid', 'precision_weighted_train', 'precision_weighted_valid', 'recall_weighted_train', 'recall_weighted_valid']

                for i in range(1, len(self._extra_scores)+1):
                    val_metrics = []
                    val_metrics.append(self._extra_scores[f'scores_{i}']['f1_weighted_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['f1_weighted_valid'])
                    
                    val_metrics.append(self._extra_scores[f'scores_{i}']['precision_weighted_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['precision_weighted_valid'])
                    
                    val_metrics.append(self._extra_scores[f'scores_{i}']['recall_weighted_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['recall_weighted_valid'])
                    
                    df[f'fold_{i}'] = val_metrics  
            elif self.main_metric in ['roc_auc_ovr', 'roc_auc_ovo']:
                df['names'] = ['auc_ovr_train', 'auc_ovr_valid', 'auc_ovo_train', 'auc_ovo_valid']

                for i in range(1, len(self._extra_scores)+1):
                    val_metrics = []
                    val_metrics.append(self._extra_scores[f'scores_{i}']['auc_ovr_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['auc_ovr_valid'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['auc_ovo_train'])
                    val_metrics.append(self._extra_scores[f'scores_{i}']['auc_ovo_valid'])
                    df[f'fold_{i}'] = val_metrics  
